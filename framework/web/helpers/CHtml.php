	...
	public static function listOptions($selection,$listData,&$htmlOptions)
	{
		...
		if(is_array($selection))
		{
			...
			$selection=$selection->$key;
                ...
                /*
                 * Моя доработка, проверка на переданное название метода для заполнения справочника
                 * 
                 */
                if (is_string($listData) && preg_match('^.*::.*^', $listData)>0)
                {
                    $function = explode('=', $listData);
                   // $listData = Reference::model()->getReference($function[1]);
                    $listData = call_user_func($function[0],$function[1]);
                }
